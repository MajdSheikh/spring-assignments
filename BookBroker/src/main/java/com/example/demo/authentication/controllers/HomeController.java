...
public String bookMarket(Model model, HttpSession session) {
    ...
    // get the user who is in the session
    Long user_id = (Long) session.getAttribute("user_id");
	...
	List<Book> borrowedBooks = userServ.findUserById(user_id).getBorrowedBooks();
...
public String borrowBook(@PathVariable("id") Long id, HttpSession session) {
	 if (session.getAttribute("user_id") != null) {
		 ...
		 borrowedBook.setBorrower(userServ.findUserById(user_id));
